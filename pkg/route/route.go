...
func NewRoute(customer listing.CustomerHandler, product listing.ProductHandler, cart listing.CartHandler) *httprouter.Router{
	...
	route.PUT("/api/cart/", cart.GetAllItem)
	route.PUT("/api/cart/:cartId", cart.RemoveItem)
	route.PUT("/api/cart/:cartId", cart.AddItem)
